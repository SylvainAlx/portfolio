export const rooms = [
  {
    className: "bridge",
    title: "Salle de contrôle",
    description: "Parcours et ambitions",
    section_1: "d'où je viens",
    section_1_text: [
      "2023 - BAC+2 Developpeur web full-stack",
      "2017-2022 - Assistant d'éducation",
      "2017-2019 - Photographe freelance",
      "2011-2016 - Technicien Canal+",
      "2010 - BAC+2 Audiovisuel",
    ],
    section_2: "postes visés",
    section_2_text: [
      "développeur web fullstack React/Node.js",
      "developpeur WordPress",
      "developpeur Front-End",
      "intégrateur web",
    ],
    button: "TÉLÉCHARGER MON CV",
    button_link: "",
  },
  {
    className: "engine",
    title: "Salle des machines",
    description: "Technologies utilisées",
  },
  {
    className: "crew",
    title: "Quartier de l'équipage",
    description: "Mes réalisations",
  },
  {
    className: "laboratory",
    title: "Laboratoire",
    description: "En cours d'apprentissage",
  },
];

export const otherRooms = [
  {
    className: "observatory",
    title: "Observatoire",
    description: "Ma passion pour l'astronomie",
  },
  {
    className: "photo",
    title: "Studio photo",
    description: "Ma pratique de la photographie",
  },
  {
    className: "music",
    title: "Salle de musique",
    description: "Je m'évade aussi en musique",
  },
  {
    className: "greenhouse",
    title: "Serre",
    description: "Un jardin comme cadre de vie",
  },
];
